...
class Generador_datos():
	...
	def generar_datos(self):
		...
		if self.datos["email"] == None:
			arroba = p[9].index("@")
			self.datos["email"] = p[9][:arroba] + str(random.randint(0,1990)) +  "@ehd4k.info"
		#Generar contraseña
		...
		self.datos["tarjeta"] = Generar_tarjeta(self.datos["BIN"],1,self.datos["base_tarjetas"]).dic_tarjetas[0]
		if(self.PRINT):
	...
	def persona_azar(self):
		db = sqlite3.connect("datos/datos_gente.db")
		cur = db.cursor()
